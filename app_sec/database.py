...
def add_user(user_email, user_password, user_username, totp_secret):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    c.execute("INSERT INTO Users (email, password, username, totp_secret) VALUES (?, ?, ?, ?)", 
              (user_email, user_password, user_username, totp_secret))
    conn.commit()
    conn.close()
...
def update_cart(user, product_id, quantity):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    c.execute("UPDATE Cart SET quantity = ? WHERE user = ? AND id = ?", (quantity, user, product_id))
    conn.commit()
    conn.close()
...
def get_total_price(user):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    # Multiply price by quantity for each item and sum up the results
    c.execute("SELECT SUM(price * quantity) FROM Cart WHERE user = ?", (user,))
    total = c.fetchone()
    conn.close()
    return total[0] if total else 0  # Return 0 if the total is None
